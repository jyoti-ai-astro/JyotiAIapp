...
const nav = [
  ['Overview', '/'], ['Users', '/users'], ['Payments', '/payments'], ['Subscriptions', '#'], ['Tickets', '#'],
  ...
  return (
    <div className="shell">
      <aside className="sidebar">
        ...
        <nav className="nav">
          {nav.map(([item, href]) => (
            <Link className={item === 'Overview' ? 'active' : ''} href={href} key={item}>{item}</Link>
          ))}
        </nav>
      </aside>
      ...
      <main className="main">
        <header className="topbar">
          <div><div className="eyebrow">Secure control plane</div><h1>Overview</h1><p className="muted">Canonical operational state for JyotiAI.</p></div>
        ...
        <section className="metric-grid">
          <article className="metric-card"><span>Total users</span><strong>{number(stats.users?.total)}</strong><small>+{number(stats.users?.newToday)} today</small></article>
          <article className="metric-card"><span>Verified revenue</span><strong>{currency(stats.payments?.verifiedRevenueTotal)}</strong><small>{currency(stats.payments?.verifiedRevenueToday)} today</small></article>
          <article className="metric-card"><span>Active subscriptions</span><strong>{number(stats.subscriptions?.active)}</strong><small>Canonical subscription records</small></article>
          <article className="metric-card"><span>Outstanding tickets</span><strong>{number(ticketTotal)}</strong><small>Current entitlement liability</small></article>
        </section>

        <section className="panel-grid">
          <article className="card"><div className="section-title">Payments</div><div className="rows"><div><span>Successful</span><strong>{number(stats.payments?.successful)}</strong></div><div><span>Failed</span><strong>{number(stats.payments?.failed)}</strong></div><div><span>Revenue source</span><strong>Verified success only</strong></div></div></article>
          <article className="card"><div className="section-title">Ticket liability</div><div className="rows"><div><span>AI Guru</span><strong>{number(stats.tickets?.aiGuruTickets)}</strong></div><div><span>Kundali</span><strong>{number(stats.tickets?.kundaliTickets)}</strong></div><div><span>Predictions</span><strong>{number(stats.tickets?.lifetimePredictions)}</strong></div></div></article>
          <article className="card"><div className="section-title">Usage today</div><div className="rows"><div><span>Reports</span><strong>{number(stats.reports?.today)}</strong></div><div><span>Guru messages</span><strong>{number(stats.guru?.usageToday)}</strong></div><div><span>New users</span><strong>{number(stats.users?.newToday)}</strong></div></div></article>
