...
export interface AstralTrailMotionState {
  time: number;
...
export function useAstralTrailMotion(): AstralTrailMotionState {
  const stateRef = useRef<AstralTrailMotionState>({
    time: 0,
    scrollProgress: 0,
    bassMotion: 0,
    midMotion: 0,
