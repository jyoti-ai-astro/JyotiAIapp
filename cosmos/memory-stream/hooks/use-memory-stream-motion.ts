...
export interface MemoryStreamMotionState {
  time: number;
...
export function useMemoryStreamMotion(): MemoryStreamMotionState {
  const stateRef = useRef<MemoryStreamMotionState>({
    time: 0,
    scrollProgress: 0,
    bassMotion: 0,
    midMotion: 0,
