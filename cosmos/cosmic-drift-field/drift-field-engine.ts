/**
 * Cosmic Drift Field Engine
 * 
 * Phase 2 — Section 45: COSMIC DRIFT FIELD ENGINE
 * Cosmic Drift Field Engine (E49)
 * 
 * Generate nebula plane, flow plane, drift dust particles, manage uniforms
 */

import * as THREE from 'three';
import { driftFieldVertexShader } from './shaders/drift-field-vertex';
import { driftFieldFragmentShader } from './shaders/drift-field-fragment';

export interface CosmicDriftFieldEngineConfig {
  intensity?: number;
...
export interface NebulaPlaneData {
  width: number;
  height: number;
  segments: number;
}

export interface FlowPlaneData {
  width: number;
  height: number;
  segments: number;
}
...
export class CosmicDriftFieldEngine {
  ...
  constructor(config: CosmicDriftFieldEngineConfig = {}) {
    this.config = {
      intensity: config.intensity || 1.0,
      mouse: config.mouse || { x: 0, y: 0 },
      parallaxStrength: config.parallaxStrength || 1.0,
      numParticles: config.numParticles || 205,
    };
    
    this.numParticles = this.config.numParticles || 205;
    
    // Generate nebula plane
    this.generateNebulaPlane();
    
    // Generate flow plane
    this.generateFlowPlane();
    ...
    this.material = new THREE.ShaderMaterial({
      ...
      vertexShader: driftFieldVertexShader,
      fragmentShader: driftFieldFragmentShader,
      transparent: true,
   ...
   * Generate nebula plane
   */
  private generateNebulaPlane(): void {
    this.nebulaPlane = {
      width: 24.0,
      height: 14.0,
      segments: 40, // 40×40 grid
    };
  }
   ...
   * Generate flow plane
   */
  private generateFlowPlane(): void {
    this.flowPlane = {
      width: 24.0,
      height: 14.0,
      segments: 40,
  ...
  /**
   * Generate dust particles (150-260)
   */
  private generateDustParticles(): void {
    this.dustParticles = [];
    
    for (let i = 0; i < this.numParticles; i++) {
      const baseX = (i / this.numParticles) * 24.0 - 12.0; // -12 to 12 range
      const speed = 0.15 + (i / this.numParticles) * 0.1; // Varying speeds
  ...
  /**
   * Create geometry with nebula plane, flow plane, and dust particles
   */
  private createGeometry(): THREE.BufferGeometry {
    ...
    for (let i = 0; i <= planeSegments; i++) {
      for (let j = 0; j <= planeSegments; j++) {
        ...
        nebulaIndices.push(0); // Single nebula plane
        flowIndices.push(-1);
        dustIndices.push(-1);
      }
    }
    ...
    for (let i = 0; i <= planeSegments; i++) {
      for (let j = 0; j <= planeSegments; j++) {
        ...
        nebulaIndices.push(-1);
        flowIndices.push(0); // Single flow plane
        dustIndices.push(-1);
      }
    }
    ...
    for (let i = 0; i < this.numParticles; i++) {
      ...
      // Indices
      nebulaIndices.push(-1, -1, -1, -1);
      flowIndices.push(-1, -1, -1, -1);
    ...
    geometry.setAttribute('nebulaIndex', new THREE.Float32BufferAttribute(nebulaIndices, 1));
    geometry.setAttribute('flowIndex', new THREE.Float32BufferAttribute(flowIndices, 1));
  ...
  /**
   * Update with cosmic drift field state
   */
  update(
    time: number,
