 ...
 * - Type-safe environment variable access (runtime only)
 */
...
// IMPORTANT: this was missing and caused client-side crashes
...
const envSchema = z.object({
  ...
  FIREBASE_ADMIN_CLIENT_EMAIL: z
    .string()
    .email('Firebase admin client email must be valid email')
    .optional(),

  // AI Providers
  AI_PROVIDER: z.enum(['openai', 'gemini', 'xai', 'anthropic']).default('openai'),
  EMBEDDING_PROVIDER: z.enum(['openai', 'gemini']).default('openai'),
  OPENAI_API_KEY: z.string().optional(),
  GEMINI_API_KEY: z.string().optional(),
  XAI_API_KEY: z.string().optional(),
  ANTHROPIC_API_KEY: z.string().optional(),
  XAI_MODEL_NAME: z.string().default('grok-4.6'),
  ANTHROPIC_MODEL_NAME: z.string().default('claude-sonnet-5'),
  GURU_MODEL_NAME: z.string().default('gpt-4.1'),
  PREDICTION_MODEL_NAME: z.string().default('gpt-4.1'),
  ...
  GURU_RAG_ENABLED: z
    ...
    .default('true'), // Super Phase C

  // ZeptoMail
  ZEPTO_API_URL: z.string().url().default('https://api.zeptomail.in/v1.1/email'),
  ZEPTO_API_TOKEN: z.string().optional(),
  ZEPTO_DOMAIN: z.string().default('jyotiai.in'),
  SENDER_EMAIL: z.string().email().default('order@jyotiai.in'),
...
function validateEnv() {
  ...
  if (!parsed.success) {
    ...
    // Production/Vercel - never throw on client, log selectively
    if ((isProduction || isVercel) && !isBuild) {
      if (isClient) {
        // Client: minimize noise, only log Firebase issues if truly missing
        ...
        if (firebaseErrors.length > 0) {
          ...
          if (!hasFirebaseVars) {
            console.warn(
              '⚠️  Firebase environment variables missing. Authentication features will not work.'
            );
          }
        }
      } else {
        // Server: log non-Firebase/non-admin errors
    ...
    if (isBuild && errors.length > 0) {
      console.warn(
        '⚠️  Environment variable validation warnings during build (will be validated at runtime):'
      );
      errors.forEach((error) => {
        console.warn(`  - ${error.path.join('.')}: ${error.message}`);
      });
...
function validateRequiredVars() {
  const required = [
    // Firebase vars are validated at runtime in Firebase config now
  ...
  if (env.AI_PROVIDER === 'xai' && !env.XAI_API_KEY) {
    missing.push('XAI_API_KEY (required when AI_PROVIDER=xai)');
  }
...
export const envVars = {
  ...
  ai: {
    ...
    xaiModelName: env.XAI_MODEL_NAME,
    anthropicModelName: env.ANTHROPIC_MODEL_NAME,
    guruModelName: env.GURU_MODEL_NAME,
    predictionModelName: env.PREDICTION_MODEL_NAME,
  },
  ...
  // Guru RAG
  guruRag: {
    enabled: env.GURU_RAG_ENABLED,
  },

  // ZeptoMail
  zepto: {
    apiUrl: env.ZEPTO_API_URL,
