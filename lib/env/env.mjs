...
function validateEnv() {
  ...
  if (!parsed.success) {
    ...
    // Phase 31 - F46: Production mode - log errors but don't crash (graceful degradation)
    if ((isProduction || isVercel) && !isBuild) {
      // Filter out Firebase errors (they're now optional)
      ...
      if (criticalErrors.length > 0) {
        ...
        throw new Error(
          `Invalid environment variables: ${criticalErrors.map(e => e.path.join('.')).join(', ')}`
        );
      } else {
