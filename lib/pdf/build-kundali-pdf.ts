...
export async function buildKundaliPDF(data: KundaliPDFData): Promise<Uint8Array> {
  ...
  pdf.addParagraph('Generated by JyotiAI', { fontSize: 10, color: [0.7, 0.7, 0.7] });

  // Birth Chart Summary
  pdf.addPage();
  pdf.addSectionTitle('Birth Chart Summary', '🌌');
  ...
  if (data.majorPlanets && data.majorPlanets.length > 0) {
    pdf.addSectionTitle('Planetary Positions', '🪐');
  ...
  if (data.houses && data.houses.length > 0) {
    pdf.addSectionTitle('Houses Analysis', '🏛️');
