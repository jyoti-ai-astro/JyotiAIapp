  ...
  return withAdminAuth(async (req) => {
    ...
    try {
      ...
      paymentDocs.forEach((doc: any) => {
        ...
        if (paymentStatus !== 'completed' && paymentStatus !== 'success') return
        ...
        revenueBySource.set(source, (revenueBySource.get(source) || 0) + safeAmount)
      })
      ...
      for (const event of events) {
        ...
        const source = String(
          event.attribution?.latestTouch?.utm_source ||
          event.attribution?.firstTouch?.utm_source ||
          (event.attribution?.latestTouch?.referrer ? 'Referral' : 'Direct / unknown')
        )
