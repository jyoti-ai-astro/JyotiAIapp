...
export async function GET(request: NextRequest) {
  return withAdminAuth(async (req) => {
    ...
    try {
      ...
      let newUsers = 0, newUsersToday = 0
      ...
      let verifiedRevenue = 0, verifiedRevenueToday = 0, successful = 0, failed = 0, pending = 0
      ...
      paymentsSnap.forEach((doc) => {
        ...
        if (data.status === 'success') {
          ...
          current.purchases += 1; current.revenue += safeAmount; products.set(product, current)
        } else if (data.status === 'failed') failed += 1
        else if (data.status === 'pending' || data.status === 'created') pending += 1
      })

      let activeSubscriptions = 0, expiring7d = 0, expiring30d = 0, cancelled = 0
      ...
      const events = analyticsSnap ? analyticsSnap.docs.map((doc) => doc.data()).filter((event) => {
        const occurred = toDate(event.receivedAt || event.occurredAt); return !occurred || occurred >= since
      }) : []
      ...
      return NextResponse.json({
        success: true, days,
        stats: {
          users: { total: usersCount.data().count, newInWindow: newUsers, newToday: newUsersToday },
          payments: { verifiedRevenue, verifiedRevenueToday, successful, failed, pending, averageOrderValue: successful ? verifiedRevenue / successful : 0, successRate: (successful + failed + pending) ? successful / (successful + failed + pending) : 0, topProducts: Array.from(products.entries()).map(([product, value]) => ({ product, ...value })).sort((a,b) => b.revenue-a.revenue).slice(0,8) },
          subscriptions: { active: activeSubscriptions, expiring7d, expiring30d, cancelled },
          growth: { visitors, sessions, landingViews: countEvent('landing_viewed'), pricingViews: countEvent('pricing_viewed'), checkoutIntent },
          attention: { failedPayments: failed, pendingPayments: pending, expiringSubscriptions7d: expiring7d },
          system: { dataSource: 'canonical', financialAuthority: 'provider-verified successful payments only' },
        },
      })
    } catch (error) {
