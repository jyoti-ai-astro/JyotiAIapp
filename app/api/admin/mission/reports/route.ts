    ...
    try {
      ...
      const reports = snapshot.docs.map((doc: any) => {
        ...
        return {
          ...
          createdAt: toIso(data.createdAt), updatedAt: toIso(data.updatedAt), completedAt: toIso(data.completedAt),
          error: data.error || data.errorMessage || null,
          storageUrl: data.storageUrl || data.downloadUrl || data.pdfUrl || null,
        }
      }).filter((report: any) => !search || [report.id, report.userId, report.type, report.title, report.status].filter(Boolean).some((value) => String(value).toLowerCase().includes(search))).slice(0, limit)
