...
function createRingGeometry(radius: number, yScale = 0.42, segments = 144) {
...
function SceneContents({ paused }: { paused: boolean }) {
  ...
  const materials = useMemo(() => {
    ...
    const orbit = new THREE.LineBasicMaterial({ color: '#d6a84f', transparent: true, opacity: 0.22 });
    const sacred = new THREE.LineBasicMaterial({ color: '#edd6a2', transparent: true, opacity: 0.13 });
    const planet = [
  ...
  useFrame((state, delta) => {
    ...
    if (sunRef.current) sunRef.current.rotation.y += step * 0.045;
    if (coronaRef.current) coronaRef.current.scale.setScalar(1.48 + Math.sin(elapsed * 0.68) * 0.035);
    if (planetRootRef.current) planetRootRef.current.rotation.z += step * 0.024;
    if (starsRef.current) starsRef.current.rotation.y += step * 0.006;
    ...
    <group ref={rootRef} position={[0.85, 0.06, 0]}>
      ...
      <group rotation={[0.18, -0.18, -0.12]}>
        ...
        {lineObjects.orbits.map((line, index) => (
          <primitive key={index} object={line} rotation={[0.18, 0.06 * index, 0]} />
        ))}
        ...
        <group ref={planetRootRef}>
          {planets.map((planet, index) => (
            <mesh
              ...
              position={[
                Math.cos(planet.offset) * planet.radius,
                Math.sin(planet.offset) * planet.radius * 0.42,
                planet.z,
              ]}
            >
      ...
      <pointLight position={[1.5, 1.5, 2]} intensity={3.2} color="#f9b15c" />
    </group>
  );
}

export default function SolarObservatoryScene() {
