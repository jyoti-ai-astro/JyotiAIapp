...
export function UpgradeBanner({
  buttonText = "Upgrade to JyotiAI Premium",
  description = "Unlock 12-month predictions, full timeline & Guru priority access",
  onClose,
  onClick,
  className,
}: UpgradeBannerProps) {
    ...
    <div className={cn("mx-auto flex items-center justify-center", className)}>
      <AnimatePresence>
        ...
        >
          ...
          >
            <SettingsFilled className="text-indigo-500" />
          </motion.div>
          <motion.div
            initial="hidden"
            animate={isHovered ? "visible" : "hidden"}
          ...
          >
            <SettingsFilled className="text-sky-500" />
          </motion.div>
          <div className="relative flex h-[35px] items-center gap-1 rounded-[6px] border border-indigo-200/70 bg-indigo-50/95 pl-2.5 pr-1 text-sm dark:border-indigo-500/60 dark:bg-indigo-950/70">
            <button
              className="focus-visible:shadow-focus-ring rounded-xs my-[-1px] cursor-pointer border-none bg-transparent px-0 py-1 font-sans text-[13px] font-medium text-indigo-900 underline decoration-indigo-200 underline-offset-[5px] outline-none hover:text-indigo-600 hover:decoration-indigo-300 dark:text-indigo-50 dark:decoration-indigo-600 dark:hover:text-indigo-300 dark:hover:decoration-indigo-400"
            ...
            <span className="text-[0.8125rem] text-indigo-700 dark:text-indigo-300">
              {description}
            </span>
            {onClose && (
              <button
                onClick={onClose}
                className="m-0 flex h-6 w-6 shrink-0 cursor-pointer items-center justify-center rounded-[4px] border-0 bg-transparent p-0 text-indigo-600 hover:bg-indigo-100 dark:hover:bg-indigo-900"
              >
                <X size={16} className="text-indigo-600 dark:text-indigo-300" />
              </button>
            )}
          </div>
        </motion.div>
